...
namespace plume {
    ...
    static constexpr size_t PUSH_CONSTANT_MAX_INDEX = 5;
    ...
    void MetalCommandList::setComputePushConstants(uint32_t rangeIndex, const void *data, const uint32_t offset, const uint32_t size) {
        ...
        pushConstants[rangeIndex].size = range.size + 4;
    ...
    void MetalCommandList::checkForUpdatesInGraphicsState() {
        ...
        if (dirtyGraphicsState.vertexBuffers) {
            std::vector<bool> slotUsed(32, false);
            ...
            for (uint32_t i = 0; i < viewCount; i++) {
                ...
                slotUsed[PUSH_CONSTANT_MAX_INDEX + 1 + vertexBufferIndices[i]] = true;
            }
